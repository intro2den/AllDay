...
bool FontShaderClass::Initialize(ID3D11Device* device, HWND hwnd){
	...
	result = InitializeShader(device, hwnd, "../rastertekTutorials/font.vs", "../rastertekTutorials/font.ps");
	if (!result){
		return false;
	}
...
bool FontShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* psFilename){
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "FontVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if (FAILED(result)){
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "FontPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if (FAILED(result)){
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage){
