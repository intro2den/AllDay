...
bool ApplicationClass::RenderGraphics(){
	...
	case MAINSTATE_COMBATMAP:
		...
		if (fmod(cursorX, 1.5f * HEX_SIZE) >= 0.5f * HEX_SIZE){
			...
			normalizedCursorX = fmod(cursorX, 1.5f * HEX_SIZE) / (0.5f * HEX_SIZE);
			normalizedCursorY = (fmod(cursorY + 0.5f * HEX_HEIGHT * (float)(hexX % 2), HEX_HEIGHT) - 0.5f * HEX_HEIGHT) / (0.5f * HEX_HEIGHT);
		...
		hexY = (int)((cursorY - 0.5 * HEX_HEIGHT * (hexX % 2)) / HEX_HEIGHT);
		...
		highlightY = (int)(HEX_HEIGHT * (hexY + 0.5 * (hexX % 2)));
