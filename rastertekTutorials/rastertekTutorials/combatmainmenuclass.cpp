...
bool CombatMainMenuClass::Initialize(int screenWidth, int screenHeight, BitmapClass* texture, BitmapClass* standardButtonTexture, TextureShaderClass* textureShader, FontClass* font, FontShaderClass* fontShader, D3DXMATRIX viewMatrix, ID3D11Device* device, ID3D11DeviceContext* deviceContext){
	...
	result = m_Text->AddText(m_menuX + textXOffset, m_menuY + textYOffset, "Close Menu", 0.0f, 0.0f, 0.0f, buttonWidth - 30, device, deviceContext);
	if (!result){
		return false;
	}

	result = m_Text->AddText(m_menuX + textXOffset, m_menuY + textYOffset + buttonSpacing, "Exit to Main Menu", 0.0f, 0.0f, 0.0f, buttonWidth - 30, device, deviceContext);
	if (!result){
		return false;
	}
